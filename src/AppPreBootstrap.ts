        ...
        AppPreBootstrap.getApplicationConfig(() => {
            ...
            if (queryStringObj.redirect && queryStringObj.redirect === 'TenantRegistration') {
                ...
                AppPreBootstrap.impersonatedAuthenticate(
                    queryStringObj.impersonationToken, queryStringObj.tenantId, () => { AppPreBootstrap.getUserConfiguration(callback); });
            } else if (queryStringObj.switchAccountToken) {
                AppPreBootstrap.linkedAccountAuthenticate(
                    queryStringObj.switchAccountToken, queryStringObj.tenantId, () => { AppPreBootstrap.getUserConfiguration(callback); });
            } else {
    ...
    private static getUserConfiguration(callback: () => void): JQueryPromise<any> {
        ...
        return abp.ajax({
            ...
            AppConsts.subscriptionExpireNootifyDayCount
                = parseInt(abp.setting.get('App.TenantManagement.SubscriptionExpireNotifyDayCount'), 10);
        });
    }
